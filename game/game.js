...
class Game {
  ...
  redTell(player, hint) {
    if (
      this._validateGameTurn(gp.RED_TELL) &&
      this._validatePlayerPosition(player)
    ) {
      this.redHint = hint
  ...
  blueTell(player, hint) {
    if (
      this._validateGameTurn(gp.BLUE_TELL) &&
      this._validatePlayerPosition(player)
    ) {
      this.blueHint = hint
  ...
  redGuess(player, word) {
    if (
      this._validateGameTurn(gp.RED_GUESS) &&
      this._validatePlayerPosition(player)
    ) {
  ...
  blueGuess(player, word) {
    if (
      this._validateGameTurn(gp.BLUE_GUESS) &&
      this._validatePlayerPosition(player)
    ) {
      this._revealCard(word)
      this._logGuess(player, this._findCard(word))
  ...
  pass(player) {
    if (
      this._validateGameTurn(gp.BLUE_GUESS, gp.RED_GUESS) &&
      this._validatePlayerPosition(player)
    ) {
      this._logPass(player)
      if (this.turn === gp.BLUE_GUESS) this.turn = gp.RED_TELL
      if (this.turn === gp.RED_GUESS) this.turn = gp.BLUE_TELL
  ...
  _computeWinner() {
    ...
    revealed.forEach(c => {
      if (c.type === assassin) assassins++
      if (c.type === red) reds++
      if (c.type === blue) blues++
    })

    if (assassins === 1) {
      this.winner = this.turn === gp.BLUE_GUESS ? 'red' : 'blue'
      return
    }
  ...
  _findCard(w) {
    ...
    if (!card) throw new Error('Could not find card: ' + w)
    return card
  }

  _computeInitialState(cards) {
    const redCards = cards.filter(card => card.type === red).length
    if (redCards === 9) return { turn: gp.RED_TELL, first: 'red' }
    else return { turn: gp.BLUE_TELL, first: 'blue' }
  }
  ...
  getResult() {
    ...
    this.cards.forEach(c => {
      if (!c.revealed && c.type === 'red') red++
      if (!c.revealed && c.type === 'blue') blue++
    })
    return { red, blue }
  }
}
