...
class FacebookApi {
  ...
  invitePlayerMessage(playerId, fromId, roomId) {
    ...
    return this.api.sendQuickReply(
      playerId,
      `You are invited to room ${roomId}`,
      replies
    )
  }
  ...
  playerRejectedInviteMessage(playerId, rejector) {
    return this.api.sendTextMessage(
      playerId,
      t.PLAYER_REJECTED_INVITE(rejector)
    )
  }
  ...
  showBoardHintMessage(playerId, game) {
    return this.api.sendTextMessage(
      playerId,
      t.BOARD_HINT(game.cards, game.getResult())
    )
  }
  showBoardGuessMessage(playerId, game) {
    return this.api.sendTextMessage(
      playerId,
      t.BOARD_GUESS(game.cards, game.getResult())
    )
  }
  ...
  turnChangedMessage(playerId, playerInTurn) {
    return this.api.sendTextMessage(
      playerId,
      t.TURN_CHANGED(playerInTurn.getNiceName())
    )
  }
  ...
  playerJoinedMessage(playerId, player) {
    return this.api.sendTextMessage(
      playerId,
      t.PLAYER_JOINED(player.getNiceName())
    )
  }
  ...
  playerTookSlotMessage(playerId, player, slot) {
    return this.api.sendTextMessage(
      playerId,
      t.PLAYER_TOOK_SLOT(player.getNiceName(), slot)
    )
  }
